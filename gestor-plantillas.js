...
// ── Registrar funciones en la tabla de dispatch del HTML principal ──────────
// Esto permite que el HTML llame a estas funciones via _callPlantilla()
// sin depender de hoisting de function declarations de scripts dinámicos.
(function() {
  _regPlantilla('renderExtra', renderExtra);
  _regPlantilla('generarDocExtra', generarDocExtra);
  _regPlantilla('extraCambiarPropiedad', extraCambiarPropiedad);
  _regPlantilla('waFichaEnviar', waFichaEnviar);
  _regPlantilla('waFichaRenderLista', waFichaRenderLista);
  _regPlantilla('waFichaCheckChange', waFichaCheckChange);
  _regPlantilla('waFichaSetFiltro', waFichaSetFiltro);
  _regPlantilla('imprimirDocExtra', imprimirDocExtra);
  _regPlantilla('copiarDocExtra', copiarDocExtra);
  _regPlantilla('descargarWordContrato', descargarWordContrato);
  _regPlantilla('renderMigracion', renderMigracion);
  _regPlantilla('ejecutarMigracion', ejecutarMigracion);
  _regPlantilla('cambiarPlanAdmin', cambiarPlanAdmin);
  _regPlantilla('toggleAyuda', toggleAyuda);
  _regPlantilla('_numToLetras', _numToLetras);
  _regPlantilla('waFichaDeseleccionarTodas', waFichaDeseleccionarTodas);
  _regPlantilla('waFichaSeleccionarTodas', waFichaSeleccionarTodas);
  // Override DOCS_EXTRA with full version
  if (typeof DOCS_EXTRA !== 'undefined') {
    DOCS_EXTRA.length = 0;
    [
      { id:'contrato',  icon:'📝', title:'Contrato de arrendamiento', desc:'Contrato completo listo para firmar, con todos los datos del inquilino, monto, fechas y cláusulas estándar.' },
      { id:'ficha',     icon:'🏠', title:'Ficha de presentación',     desc:'Ficha comercial para publicar o presentar la propiedad: foto, características, precio y contacto.' },
      ...
      { id:'deposito',  icon:'💰', title:'Recibo de depósito',         desc:'Recibo formal del depósito en garantía recibido del inquilino.' },
      { id:'salida',    icon:'🚪', title:'Acta de devolución / salida', desc:'Documento que firmará el inquilino al dejar la propiedad, con estado final y devolución del depósito.' },
      { id:'rescision', icon:'⚖️', title:'Rescisión de contrato',      desc:'Documento formal para rescindir el contrato por incumplimiento de cláusulas o salida anticipada.' },
    ].forEach(function(d){ DOCS_EXTRA.push(d); });
  }
})();
